...
class Info:
    ...
    async def announce(self, ctx, message : str):
        ...
        failedservers = []
        devservers = ['110373943822540800', '133049272517001216', '267855028914618369', '209814152288403457']
        for s in self.bot.servers:
            if s.id in devservers:
                continue
            try:
    ...
    async def roles(self, ctx):
        ...
        list=discord.Embed(title="▬▬▬▬▬▬▬▬▬▬ஜ۩۞۩ஜ▬▬▬▬▬▬▬▬▬▬", description=", ".join([x.name for x in ctx.message.server.role_hierarchy if x.name != "@everyone"]), colour=discord.Colour.blue())
