...
class UtilsMenu:
    ...
    async def invite(self):
        """Provides my invite URL so you can add CodeRED to another server"""

        await self.bot.say("You want to invite me to your server? Aww, how sweet, always nice to know someone wants me. Here's my OAuth2 URL:\n https://discordapp.com/oauth2/authorize?client_id=258342586798964736&scope=bot&permissions=-1 \n\n Need support or want to ask a question or just want to generically chat about CodeBLUE? Go here for the official channel:\n https://discord.io/invitecodeblue")
    ...
    async def disclaimer(self, ctx):
        ...
        if answer is None:
            ...
            await self.bot.say(":warning: | ***__DISCLAIMER:__*** \n \n CodeBLUE (me :3), is made by GambaPlayz. \n He did **__NOT__** create all of the modules and has never claimed to (*__module credit goes to their respective developers__*). \n There seems to be some confusion somewhere, so I'd like to clear it up. \n \n CodeBLUE and Red **__ARE NOT__** the same thing. CodeBLUE may have some of Red's modules (props to Twentysix as he's an amazing developer) but only ones that GambaPlayz couldn't get to work or couldn't be bothered to create. \n However, the ones he has 'taken' are extensively modified to the point where you'll almost never have the same version. Quite a few things were still made by GambaPlayz though, including the base framework of the bot. \n Thanks for understanding. ~GambaPlayz ")
            return
        elif "no" in answer.content.lower():
            await self.bot.say("Oh, you found this by accident. Ok. I won't display the disclaimer. Sorry about that, didn't mean to sound aggressive, just don't use this command again, ok? :3")
